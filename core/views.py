...
class PaymentView(View):
    ...
    def post(self, *args, **kwargs):
        try:
            ...
            messages.error(self.request, "Payment service is unavailable right now.")
            return redirect("core:checkout")
    ...
    def get_queryset(self):
        ...
        if search_query:
            queryset = queryset.filter(
                ...
                Q(description_long__icontains=search_query)
            )
...
class CategoryView(View):
    def get(self, *args, **kwargs):
        ...
        # Apply search filtering within category
        search_query = self.request.GET.get('q')
        if search_query:
            items = items.filter(
                Q(title__icontains=search_query) |
...
def add_to_cart(request, slug):
    ...
    if order_qs.exists():
        ...
        # Generate a temporary ref_code
        import random
        import string
        temp_ref_code = ''.join(random.choices(string.ascii_lowercase + string.digits, k=20))
        order = Order.objects.create(
            user=request.user, 
            ordered_date=ordered_date,
            ref_code=temp_ref_code
        )
        order.items.add(order_item)
        messages.info(request, "Item was added to your cart.")
    return redirect("core:order-summary")
...
def buy_now(request, slug):
    ...
    messages.info(request, "Item added to cart. Redirecting to checkout.")
    return redirect("core:checkout")
    ...
    def post(self, *args, **kwargs):
        ...
        if form.is_valid():
            try:
                ...
                messages.info(self.request, "You do not have an active order")
                return redirect("core:checkout")
        messages.info(self.request, "Invalid coupon code input.")
        return redirect("core:checkout")
